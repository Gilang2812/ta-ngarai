const {
  ...
  getFacilityUnits,
} = require("./homestayFacility.service");
...
router.get("/details/index", async (req, res, next) => {
  try {
...
router.get("/units/index", async (req, res, next) => {
  try {
    const units = await getFacilityUnits();
